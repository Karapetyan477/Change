...
class ConvertViewController: UIViewController {
    ...
    @IBAction func enterNum(_ sender: Any) {
        guard let amout = currencyText.text, let theAmoutText = Double(amout) else {return}
        if currencyText.text  != "" {
            let total = theAmoutText * activeCurrency
            firstCount.text =  String(format: "%.2f", total)
        }
    }
    
    @objc func updateViews(input: Double){
        guard let amount = currencyText.text, let theAmoutText = Double(amount) else {return}
    if currencyText.text != "" {
        let total = theAmoutText * activeCurrency
        firstCount.text = String(format: "%.2f", total)
        }
